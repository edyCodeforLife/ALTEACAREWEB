...
const InBeetween = styled.div<any>`
    ...
    background: #cbddff;
    border-bottom-left-radius: ${props => (props.first || props.selectedStart ? '15' : '0')}px;
    border-top-left-radius: ${props => (props.first || props.selectedStart ? '15' : '0')}px;
    border-bottom-right-radius: ${props => (props.last || props.selectedEnd ? '15' : '0')}px;
    border-top-right-radius: ${props => (props.last || props.selectedEnd ? '15' : '0')}px;
    content: ' ';
    display: block;
`;
...
function _Days(props: {
	...
	const { selectedMonth, selectedYear, minDate, maxDate, selectedDate, onDateSelected } = props;

	const onHoverDate = debounce(date => (props.onHoverDate ? props.onHoverDate(date) : {}), 10);
	...
	const selectedStartInstance = useMemo(() => new Date(props.selectedStart), [props]);
	const selectedEndInstance = useMemo(() => new Date(props.selectedEnd), [props]);
	...
	const ifExistInArray = useCallback((d: any, list: any) => {
		switch (list) {
			...
			case undefined:
				return 'no';
			default:
				return !isUndefined(find(list, o => isSameDay(new Date(d), new Date(o))))
					? 'yes'
					: 'no';
		}
	}, []);
						...
						>
							...
							{isInBeetween(date) ? (
								<InBeetween
									last={dIndex === 6}
									first={dIndex === 0}
									selectedStart={isSameDay(new Date(date), selectedStartInstance)}
